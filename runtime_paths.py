...
def atomic_text_output(path):
    """Publish only a complete UTF-8 output; keep the old file on failure."""
    target = Path(path)
    fd, temporary = tempfile.mkstemp(prefix='.trace-', suffix='.tmp', dir=str(target.parent))
    try:
...
def configure_csv_field_limit():
    """Remove Python's small default field cap, within the platform C long."""
    limit = sys.maxsize
    while True:
        try:
...
def bounded_derived_path(
    ...
    limit = component_name_limit(base.parent)
    # Lossy sanitization can collide even for short identities (gen[0] and
    # gen_0, or names containing slash/colon). Preserve identity, not just length.
    lossy = readable != str(identity if readable_identity is None else readable_identity)
    if len(requested_name.encode("utf-8")) <= limit and not lossy:
        return requested
